...
class DECA(nn.Module):
    def __init__(self, config=None, device='cuda'):
        ...
        if config is None:
            ...
            self.cfg = config
        self.device = device
        self.image_size = 224 # self.cfg.dataset.image_size
        self.uv_size = 256 # self.cfg.model.uv_size
    ...
    def _create_model(self, model_cfg):
        ...
        # decoders
        self.D_detail = Generator(latent_dim=self.n_detail, out_channels=1, out_scale=model_cfg.max_z, sample_mode = 'bilinear').to(self.device)
        # resume model
    ...
    # @torch.no_grad()
    def encode(self, ftex, use_detail=True):
        codedict = {}
        codedict['ftex'] = ftex
        if use_detail:
            detailcode = self.E_detail(ftex)
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        if use_detail:
            uv_z = self.D_detail(codedict['detail'])
