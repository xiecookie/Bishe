...
class Trainer(object):
    ...
    def training_step(self, batch):
        ...
        ftex = batch['ftex'].to(self.device)
        ...
        codedict = self.deca.encode(ftex, use_detail=self.train_detail)

        batch_size = ftex.shape[0]
        ...
        # ------ rendering
        ops = self.deca.render(trans_verts, trans_verts, gttex, lightcode)

        uv_z = self.deca.D_detail(detailcode, dim=1)
        # render detail
        uv_detail_normals = self.deca.displacement2normal(uv_z, trans_verts, ops['trans_normals'])
        uv_shading = self.deca.render.add_SHlight(uv_detail_normals, lightcode.detach())
        uv_texture = ftex.detach() * uv_shading
