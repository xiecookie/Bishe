...
from .wrinkle import WrinkleDataset
...
def build_train(config, is_train=True):
    data_list = []
    if 'wrinkle' in config.training_data:
#         data_list.append(WrinkleDataset(image_size=config.image_size))
        return WrinkleDataset(image_size=config.image_size)
